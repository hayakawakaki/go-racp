...
func (r *Repository) Create(ctx context.Context, ticket domain.Ticket, initial domain.Message) (int64, error) {
...
func scanTicket(row pgx.Row) (domain.Ticket, error) {
	...
	if err != nil {
		return domain.Ticket{}, fmt.Errorf("infra.scanTicket: %w", err)
	}
	ticket.Status = domain.Status(status)
	ticket.LastActor = domain.Actor(lastActor)
	ticket.ClosedBy = closedBy

	return ticket, nil
}

func (r *Repository) Get(ctx context.Context, id int64) (domain.Ticket, error) {
	...
	if err != nil {
		return domain.Ticket{}, fmt.Errorf("infra.Repository.Get: %w", err)
	}

	return ticket, nil
}

func (r *Repository) ListForPlayer(ctx context.Context, accountID int, page domain.Page) ([]domain.Ticket, int, error) {
	var total int
...
func (r *Repository) ListForStaff(ctx context.Context, tab domain.StaffTab, categoryKeys []string, page domain.Page) ([]domain.Ticket, int, error) {
...
func staffTabWhere(tab domain.StaffTab, categoryKeys []string) (where string, args []any) {
	catList, catArgs := categoryInClause(categoryKeys, 1)
	base := `category IN (` + catList + `)`
	switch tab {
	case domain.TabOpenNoResponse:
...
func (r *Repository) AppendPublicMessage(ctx context.Context, ticketID int64, message domain.Message) (domain.Ticket, error) {
	tx, err := r.Pool.Begin(ctx)
	if err != nil {
		return domain.Ticket{}, fmt.Errorf("infra.Repository.AppendPublicMessage: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	...
	if _, execErr := tx.Exec(ctx,
		...
		return domain.Ticket{}, fmt.Errorf("infra.Repository.AppendPublicMessage: %w", execErr)
	}
...
func (r *Repository) AppendInternalNote(ctx context.Context, ticketID int64, message domain.Message) error {
...
func (r *Repository) AppendSystemEvent(ctx context.Context, ticketID int64, updated domain.Ticket, message domain.Message) error {
	tx, err := r.Pool.Begin(ctx)
	if err != nil {
...
func (r *Repository) SetTerminal(ctx context.Context, ticketID int64, status domain.Status, staffID int, at time.Time) (domain.Ticket, domain.Message, error) {
	tx, err := r.Pool.Begin(ctx)
	if err != nil {
		return domain.Ticket{}, domain.Message{}, fmt.Errorf("infra.Repository.SetTerminal: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	...
	if insertErr := tx.QueryRow(ctx,
		...
		return domain.Ticket{}, domain.Message{}, fmt.Errorf("infra.Repository.SetTerminal: %w", insertErr)
	}
	...
	updated, err := scanTicket(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.Ticket{}, domain.Message{}, domain.ErrTicketNotFound
	}
	if err != nil {
		return domain.Ticket{}, domain.Message{}, fmt.Errorf("infra.Repository.SetTerminal: %w", err)
	}
	...
	return updated, domain.Message{
		ID:         messageID,
		TicketID:   ticketID,
		AuthorID:   staffID,
		AuthorRole: domain.ActorStaff,
		Visibility: domain.VisibilitySystem,
		Body:       fallback,
		Event:      eventJSON,
		CreatedAt:  at,
	}, nil
