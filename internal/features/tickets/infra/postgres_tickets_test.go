...
var _ domain2.Repository = (*Repository)(nil)
...
func openTicketFixture(accountID int, now time.Time) (domain2.Ticket, domain2.Message) {
	ticket := domain2.Ticket{
		AccountID:      accountID,
		AuthorUsername: "alice",
		Category:       "Other",
		Subject:        "hello",
		Status:         domain2.StatusOpen,
		LastActor:      domain2.ActorPlayer,
		MessageCount:   1,
		LastActivity:   now,
		CreatedAt:      now,
	}
	message := domain2.Message{
		AuthorID:   accountID,
		AuthorRole: domain2.ActorPlayer,
		Visibility: domain2.VisibilityPublic,
		Body:       "body",
		CreatedAt:  now,
	}
...
func TestRepository_AppendPublicMessage_UpdatesDenormalizedFields(t *testing.T) {
	...
	if updated.LastActor != domain2.ActorStaff {
...
func TestRepository_ListForStaff_FiltersByTab(t *testing.T) {
	...
	awaiting, total, err := repo.ListForStaff(context.Background(),
		domain2.TabOpenNoResponse, []string{"Other"}, domain2.Page{Limit: 20, Offset: 0})
	if err != nil {
	...
	active, total, err := repo.ListForStaff(context.Background(),
		domain2.TabActive, []string{"Other"}, domain2.Page{Limit: 20, Offset: 0})
	if err != nil {
...
func TestRepository_SetTerminal(t *testing.T) {
	...
	closed, _, err := repo.SetTerminal(context.Background(), id, domain2.StatusClosed, 9, now.Add(time.Hour))
	if err != nil {
		t.Fatalf("SetTerminal: %v", err)
	}
	if closed.Status != domain2.StatusClosed || closed.ClosedBy == nil || *closed.ClosedBy != 9 {
