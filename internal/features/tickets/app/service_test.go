...
type fakeRepo struct {
	tickets    map[int64]domain2.Ticket
...
func newFakeRepo() *fakeRepo {
	return &fakeRepo{tickets: map[int64]domain2.Ticket{}, nextID: 1}
}

func (r *fakeRepo) Create(_ context.Context, ticket domain2.Ticket, _ domain2.Message) (int64, error) {
	id := r.nextID
	r.nextID++
	ticket.ID = id
	r.tickets[id] = ticket
...
func (r *fakeRepo) Get(_ context.Context, id int64) (domain2.Ticket, error) {
...
func (r *fakeRepo) ListForPlayer(context.Context, int, domain2.Page) ([]domain2.Ticket, int, error) {
	return nil, 0, nil
}

func (r *fakeRepo) ListForStaff(context.Context, domain2.StaffTab, []string, domain2.Page) ([]domain2.Ticket, int, error) {
	return nil, 0, nil
}
...
func (r *fakeRepo) AppendPublicMessage(_ context.Context, id int64, message domain2.Message) (domain2.Ticket, error) {
	ticket := r.tickets[id]
...
func (r *fakeRepo) AppendInternalNote(context.Context, int64, domain2.Message) error { return nil }

func (r *fakeRepo) AppendSystemEvent(_ context.Context, id int64, updated domain2.Ticket, _ domain2.Message) error {
	r.tickets[id] = updated
	return nil
}

func (r *fakeRepo) SetTerminal(_ context.Context, id int64, status domain2.Status, staffID int, at time.Time) (domain2.Ticket, domain2.Message, error) {
	ticket := r.tickets[id]
	...
	return ticket, domain2.Message{}, nil
}

type fakeMessages struct{}

func (f fakeMessages) List(context.Context, int64, bool) ([]domain2.Message, error) {
	return nil, nil
}
...
func newService(repo *fakeRepo, mailer *fakeMailer, users fakeUsers) *Service {
	resolver := domain2.NewCategoryResolver([]domain2.Category{
...
func TestOpenTicket_BlockedByMaxOpen(t *testing.T) {
	...
	if !errors.Is(err, domain2.ErrTooManyOpenTickets) {
...
func TestOpenTicket_BlockedByCooldown(t *testing.T) {
	...
	service := newService(repo, &fakeMailer{},
		fakeUsers{user: &accdomain.User{Username: "alice", Email: "a@b"}})
	_, err := service.OpenTicket(context.Background(), 1, "Other", "subj", "body")
	if !errors.Is(err, domain2.ErrTicketCooldown) {
...
func TestOpenTicket_UnknownCategory(t *testing.T) {
	...
	if !errors.Is(err, domain2.ErrUnknownCategory) {
...
func TestStaffReply_NotifiesMailer(t *testing.T) {
	...
	repo := newFakeRepo()
	repo.tickets[1] = domain2.Ticket{
		ID: 1, AccountID: 100, Status: domain2.StatusOpen, LastActor: domain2.ActorPlayer,
	}
	repo.nextID = 2
	mailer := &fakeMailer{}
...
func TestStaffNote_DoesNotNotify(t *testing.T) {
	...
	repo := newFakeRepo()
	repo.tickets[1] = domain2.Ticket{ID: 1, AccountID: 100, Status: domain2.StatusOpen, LastActor: domain2.ActorPlayer}
	mailer := &fakeMailer{}
	service := newService(repo, mailer,
		fakeUsers{user: &accdomain.User{Username: "alice", Email: "alice@example"}})
...
func TestPlayerReply_RejectsNonOwner(t *testing.T) {
	...
	repo := newFakeRepo()
	repo.tickets[1] = domain2.Ticket{ID: 1, AccountID: 100, Status: domain2.StatusOpen, LastActor: domain2.ActorStaff}
	service := newService(repo, &fakeMailer{},
		fakeUsers{user: &accdomain.User{Username: "alice", Email: "x"}})
	err := service.PlayerReply(context.Background(), 999, 1, "hi")
	if !errors.Is(err, domain2.ErrNotTicketOwner) {
...
func TestStaffResolve_NotifiesAndSetsStatus(t *testing.T) {
	...
	if repo.tickets[1].Status != domain2.StatusResolved {
