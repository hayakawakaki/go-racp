...
func TestService_ConsumePasswordReset_TierRejections(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			seedUser: func(repo *fakeUserRepo) int {
				u, _ := repo.Create(context.Background(), &domain.User{
					Username: "u", Email: "u@x", State: 0, UnbanTime: future,
				}, "Old1234!")
				return u.ID
			},
			wantErr: domain.ErrAccountTempBanned,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, repoErr := fx.userRepo.GetByID(context.Background(), accountID)
			if repoErr == nil {
				if pw := fx.userRepo.passwords[accountID]; pw != "Old1234!" {
					t.Errorf("password was updated to %q despite tier rejection", pw)
				}
			}
