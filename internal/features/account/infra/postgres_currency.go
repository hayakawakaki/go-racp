...
func (r *CurrencyRepository) lockCurrency(ctx context.Context, tx pgx.Tx, accountID int) (zeny int64, cashpoint int, withdrawLock, depositLock *time.Time, err error) {
	if _, err = tx.Exec(ctx,
		`INSERT INTO cp_currency (account_id) VALUES ($1) ON CONFLICT (account_id) DO NOTHING`, accountID,
	); err != nil {
		return 0, 0, nil, nil, fmt.Errorf("infra.CurrencyRepository.lockCurrency ensure: %w", err)
	}

	if err = tx.QueryRow(ctx,
		`SELECT zeny, cashpoint, withdraw_locked_until, deposit_locked_until FROM cp_currency WHERE account_id = $1 FOR UPDATE`, accountID,
	).Scan(&zeny, &cashpoint, &withdrawLock, &depositLock); err != nil {
		return 0, 0, nil, nil, fmt.Errorf("infra.CurrencyRepository.lockCurrency read: %w", err)
	}

	return zeny, cashpoint, withdrawLock, depositLock, nil
}
...
func (r *CurrencyRepository) CreditDeposit(ctx context.Context, depositID int64, accountID int, zeny int64, cashpoint int, withdrawLockUntil, now time.Time) (bool, error) {
	...
	currentZeny, currentCashpoint, _, depositLock, err := r.lockCurrency(ctx, tx, accountID)
	if err != nil {
		return false, fmt.Errorf("infra.CurrencyRepository.CreditDeposit lock: %w", err)
	}
	if cooldownActive(depositLock, now) {
	...
	if _, err = tx.Exec(ctx,
		`UPDATE cp_currency SET zeny = $1, cashpoint = $2, withdraw_locked_until = $3 WHERE account_id = $4`,
		newZeny, newCashpoint, withdrawLockUntil, accountID,
	); err != nil {
...
func (r *CurrencyRepository) RequestWithdraw(ctx context.Context, accountID int, zeny int64, cashpoint int, depositLockUntil, now time.Time) (int64, error) {
	tx, err := r.Pool.Begin(ctx)
	if err != nil {
	...
	currentZeny, currentCashpoint, withdrawLock, _, err := r.lockCurrency(ctx, tx, accountID)
	if err != nil {
		return 0, fmt.Errorf("infra.CurrencyRepository.RequestWithdraw lock: %w", err)
	}
	if cooldownActive(withdrawLock, now) {
	...
	if _, err = tx.Exec(ctx,
		`UPDATE cp_currency SET zeny = $1, cashpoint = $2, deposit_locked_until = $3 WHERE account_id = $4`,
		newZeny, newCashpoint, depositLockUntil, accountID,
	); err != nil {
