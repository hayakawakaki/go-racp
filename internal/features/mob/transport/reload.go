...
func (h *Handler) doReload(w http.ResponseWriter, r *http.Request) {
	...
	case errors.Is(err, refdata.ErrReloadConflict):
		...
		httpx.RenderHTML(w, r, h.logger, h.theme.MobReloadConflict())
		return
	case err != nil:
		h.logger.Error("mob: reload failed", "err", err)
		w.WriteHeader(http.StatusInternalServerError)
		httpx.RenderHTML(w, r, h.logger, h.theme.MobReloadFailure("Reload failed. Check server logs for details."))
		return
	}
	httpx.RenderHTML(w, r, h.logger, h.theme.MobReloadSuccess(h.svc.Status()))
}
