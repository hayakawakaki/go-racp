...
func newResetHandler(reset *stubResetService) *Handler {
	return &Handler{
		resetSvc: reset,
...
func TestShowResetPassword_NoToken_RendersInvalid(t *testing.T) {
	t.Parallel()
	h := newResetHandler(&stubResetService{})
...
func TestDoResetPassword_TokenErrors_RendersMatchingResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			h := newResetHandler(&stubResetService{
				consumeFn: func(context.Context, string, string) error { return tt.consumeErr },
			})
...
func TestDoResetPassword_Success_RendersSuccess(t *testing.T) {
	t.Parallel()
	h := newResetHandler(&stubResetService{
		consumeFn: func(context.Context, string, string) error { return nil },
	})
