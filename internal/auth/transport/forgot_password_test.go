...
func newForgotHandler(reset *stubResetService, logBuffer io.Writer) *Handler {
	if logBuffer == nil {
...
func TestShowForgotPassword_Renders(t *testing.T) {
	t.Parallel()
	h := newForgotHandler(&stubResetService{}, nil)
...
func TestDoForgotPassword_Success_RendersSubmittedNotice(t *testing.T) {
	t.Parallel()
	calls := 0
	reset := &stubResetService{
		requestFn: func(_ context.Context, email string) error {
			calls++
...
func TestDoForgotPassword_GenericError_LogsAndShowsGenericMessage(t *testing.T) {
	...
	reset := &stubResetService{
		requestFn: func(context.Context, string) error { return errors.New("db unreachable") },
	}
